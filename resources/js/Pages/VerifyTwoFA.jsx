...
export default function VerifyTwoFA() {
    ...
    useEffect(() => {
        ...
        if (!hasTp && uri) {
            QRCode.toDataURL(uri, { width: 200, margin: 2, color: { dark: '#0a0a0f', light: '#ffffff' } })
                .then(url => { setQrDataUrl(url); setReady(true); })
                .catch(() => setReady(true));
        } else {
            setReady(true);
        }
    ...
    if (!ready) {
        return (
            <div className="min-h-screen bg-slate-100 dark:bg-[#0a0a0f] flex items-center justify-center transition-colors duration-300">
                ...
                <div className="relative z-10 w-full max-w-sm">
                    <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl p-8 shadow-xl dark:shadow-none">
                        ...
                        <div className="flex flex-col items-center mb-8">
                            ...
                            <h1 className="text-xl font-semibold text-gray-900 dark:text-white tracking-tight">Verifikasi 2FA</h1>
                            ...
                            <p className="text-sm text-gray-400 dark:text-white/40 mt-2 text-center">
                                {hasTotp
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                                    ...
                                    <div className="w-full flex flex-col gap-2">
                                        {[
                                            ...
                                            <div key={i} className="flex items-start gap-2.5">
                                                <span className="w-5 h-5 rounded-full bg-indigo-500/20 text-indigo-500 dark:text-indigo-400 text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5">
                                                    {i + 1}
                                                </span>
                                                <p className="text-xs text-gray-500 dark:text-white/50 leading-relaxed">{step}</p>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="flex gap-2 justify-between" onPaste={handleTotpPaste}>
                                    {Array.from({ length: 6 }).map((_, i) => (
                                        <input
                                            ...
                                            className="w-full aspect-square text-center text-lg font-bold text-gray-900 dark:text-white bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl outline-none focus:border-indigo-500/70 focus:bg-indigo-50 dark:focus:bg-indigo-500/10 transition-all caret-transparent"
                                        />
                                    ))}
                                </div>
                                <p className="text-xs text-gray-400 dark:text-white/25 text-center">6 digit dari Google Authenticator</p>
                            </div>
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl transition-all duration-200 shadow-lg shadow-indigo-500/25 active:scale-[0.98] text-sm"
                            >
                                {loading ? (
                            ...
                            <button
                                ...
                                className="text-xs text-gray-400 dark:text-white/30 hover:text-gray-700 dark:hover:text-white/60 transition-colors text-center"
                            >
                                ← Kembali ke login
                            </button>
                        </form>
